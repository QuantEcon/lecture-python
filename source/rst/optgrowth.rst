...
techniques to improve execution time drastically over the next few lectures.
...
Hence, once we have a good approximation to :math:`v^*`, we can compute the optimal policy (approximately) by computing the corresponding greedy policy.
...
#. the first 36 functions generated by the fitted value function iteration algorithm, with warmer colors given to higher iterates
