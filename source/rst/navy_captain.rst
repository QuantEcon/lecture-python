...
-  Each period a decision maker draws a non-negative random variable
   ...
   :math:`f_{0}` or :math:`f_1` and that the probability that it
   selected :math:`f_0` is probability :math:`\pi^{*}`.

-  The decision maker observes a sample
   :math:`\left\{ z_{i}\right\} _{i=0}^{t}` from the the distribution
   chosen by nature.

The decision maker wants to decide which distribution actually governs
:math:`Z` and is worried by two types of errors and the losses that they
impose on him.
...
We set the cost :math:`c` of taking one more draw  at :math:`1.25`.

We set the probability distributions :math:`f_{0}` and :math:`f_{1}` to
be beta distributions with :math:`a_{0}=b_{0}=1`, :math:`a_{1}=3`, and
:math:`b_{1}=1.2`, respectively.
...
That decision rule is characterized by a sample size :math:`t` and a
cutoff :math:`d` associated with a likelihood ratio.

Let
:math:`L\left(z^{t}\right)=\prod_{i=0}^{t}\frac{f_{0}\left(z_{i}\right)}{f_{1}\left(z_{i}\right)}`
be the likelihood ratio associated with observing the sequence
:math:`\left\{ z_{i}\right\} _{i=0}^{t}`.

The decision rule associated with a sample size :math:`t` is:
...
-  alternative :math:`H_{1}`: :math:`f=f_{1}`.

Given sample size :math:`t` and cutoff :math:`d`, under the model
...
-  :math:`PFA` denotes the probability of a **false alarm**, i.e.,
   rejecting :math:`H_0` when it is true

-  :math:`PD` denotes the probability of a **detection error**, i.e.,
...
:math:`\pi^{*}=\Pr\left\{ \text{nature selects }f_{0}\right\} =0.5`.

We can solve the minimization problem in two steps.

First, we fix :math:`t` and find the optimal cutoff :math:`d` and
consequently the minimal :math:`\bar{V}_{fre}\left(t\right)`.
...
Let’s now change the value of :math:`\pi^{*}` and watch how the decision
rule changes.
...
-  accept :math:`H_0` if :math:`\pi \geq \alpha`

-  accept :math:`H_1` if :math:`\pi \leq \beta`
...
:math:`V^{0}\left(\pi\right)` and :math:`V^{1}\left(\pi\right)`
numerically.
...
:math:`\pi^{*}=\Pr\left\{ \text{nature selects }f_{0}\right\}`, we can
then compute :math:`\bar{V}_{Bayes}\left(\pi_{0}\right)`.

We can then determine an initial Bayesian prior :math:`\pi_{0}^{*}` that
minimizes this objective concept of expected loss.

The figure 9 below plots four cases corresponding to
:math:`\pi^{*}=0.25,0.3,0.5,0.7`.

We observe that in each case :math:`\pi_{0}^{*}` equals :math:`\pi^{*}`.
...
verifies the equality of :math:`\pi_{0}^{*}` to :math:`\pi^{*}` holds
for all :math:`\pi^{*}`.
...
Evidently, there is no sample size :math:`t` at which the frequentist
...
rule does better on average for all values of :math:`\pi^{*}`.
...
:math:`\bar{V}_{fre}-\bar{V}_{Bayes}`.
...
conditional on distribution :math:`q=f_{0}` or :math:`q= f_{1}`
generating the data.
...
rule when :math:`q= f_0` and **later** when :math:`q = f_1`.
...
posteriors after :math:`t` draws.

The left graph compares :math:`E\left(\pi_{t}\right)` under
:math:`f_{0}` to :math:`1-E\left(\pi_{t}\right)` under :math:`f_{1}`:
they lie on top of each other.

However, as the right hand size graph shows, there is significant
difference in variances when :math:`t` is small: the variance is lower
under :math:`f_{1}`.

The difference in variances is the reason that the Bayesian decision
maker waits longer to decide when :math:`f_{1}` generates the data.
...
possible distributions :math:`f_0` and :math:`f_1`.
...
under :math:`f_{1}` is the optimal probability of detection given
:math:`t` that we defined earlier, and similarly it equals :math:`1`
minus the optimal probability of a false alarm under :math:`f_{0}`.
...
:math:`t` *and* that the decision is correct.
...
correspond to the Bayesian cutoffs :math:`\alpha` and :math:`\beta`.
...
:math:`f_0` generating the data, the other conditional on :math:`f_1`
generating the data.
